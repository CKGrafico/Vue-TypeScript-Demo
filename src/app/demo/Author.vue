<template>
  <section class="author">
    <header class="author-header" :class="{'is-premium': author.roles.find(x => x === 'premium')}"></header>
...
export default Vue.extend({
  ...
  methods: {
    generateBadges() {
      this.generateYearBadge();
      this.generateNameBadge();
      this.generateFollowersBadge();
    },
    generateNameBadge() {
      if (!this.author.name) {
        return;
      ...
      const letter = this.author.name.slice(0, 1);
    ...
    generateYearBadge() {
      ...
      switch (this.author.registerYear) {
        case 2016:
          emoji = 'monkey';
          break;
        case 2017:
    ...
    generateFollowersBadge() {
      if (!this.author.followers) {
        return;
      }
